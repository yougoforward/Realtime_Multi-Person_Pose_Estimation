...
caffe_path = '/home/long/github/caffe/'
import sys, os
...
def setLayers_twoBranches(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in,
                          deploy=False, batchnorm=0, lr_mult_distro=[1, 1, 1]):
    ...
    if deploy == False and "lmdb" not in data_source:
        if (len(label_name) == 1):
            n.data, n.tops[label_name[0]] = L.HDF5Data(hdf5_data_param=dict(batch_size=batch_size, source=data_source),
                                                       ntop=2)
        elif (len(label_name) == 2):
            n.data, n.tops[label_name[0]], n.tops[label_name[1]] = L.HDF5Data(
                hdf5_data_param=dict(batch_size=batch_size, source=data_source), ntop=3)
    # produce data definition for deploy net
        ...
        n.tops[label_name[2]], n.tops[label_name[3]], n.tops[label_name[4]], n.tops[label_name[5]] = L.Slice(n.label,
                                                                                                             slice_param=dict(
                                                                                                                 axis=1,
                                                                                                                 slice_point=[
                                                                                                                     38,
        ...
        if layername[l] == 'C':
            ...
            for level in range(0, 2):
                if state == 'image':
                    ...
                    pool_counter, local_counter, level + 1)  # no image state in subsequent stages
                    if stage == 1:
                        lr_m = lr_mult_distro[1]
                    else:
                        lr_m = lr_mult_distro[1]
            ...
            n.tops['pool%d_stage%d' % (pool_counter, stage)] = L.Pooling(n.tops[last_layer[0]], kernel_size=kernel[l],
                                                                         stride=stride[l], pool=P.Pooling.MAX)
            ...
            if deploy == False and "lmdb" not in data_source:
                ...
                n.tops['loss_stage%d' % stage] = L.EuclideanLoss(n.tops['map_vec_stage%d' % stage],
                                                                 n.tops[label_name[1]])
            elif deploy == False:
                level = 1
                name = 'weight_stage%d' % stage
                n.tops[name] = L.Eltwise(n.tops[last_layer[level]], n.tops[label_name[(level + 2)]],
            ...
            for level in range(0, 2):
                if deploy == False and "lmdb" not in data_source:
                    ...
                    n.tops['loss_stage%d_L%d' % (stage, level + 1)] = L.EuclideanLoss(n.tops['map_vec_stage%d' % stage],
                    ...
                    n.tops[name] = L.Eltwise(n.tops[last_layer[level]], n.tops[label_name[(level + 2)]],
                                             operation=P.Eltwise.PROD)
                    n.tops['loss_stage%d_L%d' % (stage, level + 1)] = L.EuclideanLoss(n.tops[name],
                                                                                      n.tops[label_name[level]],
                                                                                      loss_weight=weight[level])
            ...
            if deploy == False:
                level = 0
                n.tops['loss_stage%d_L%d' % (stage, level + 1)] = L.Euclidean2Loss(n.tops[last_layer[level]],
                                                                                   n.tops[label_name[level]],
                                                                                   n.tops[label_name[2]],
                                                                                   loss_weight=weight[level])
                print 'loss %d level %d' % (stage, level + 1)
                level = 1
                n.tops['loss_stage%d_L%d' % (stage, level + 1)] = L.EuclideanLoss(n.tops[last_layer[level]],
                                                                                  n.tops[label_name[level]],
                                                                                  loss_weight=weight[level])
                print 'loss %d level %d' % (stage, level + 1)
            ...
            if deploy == False:
                n.tops['drop%d_stage%d' % (drop_counter, stage)] = L.Dropout(n.tops[last_layer[0]], in_place=True,
                                                                             dropout_param=dict(dropout_ratio=0.5))
                drop_counter += 1
        elif layername[l] == '@':
            # if not share_point:
            #    share_point = last_layer
            n.tops['concat_stage%d' % stage] = L.Concat(n.tops[last_layer[0]], n.tops[last_layer[1]],
                                                        n.tops[share_point], concat_param=dict(axis=1))
...
def writePrototxts(dataFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param_in, base_lr,
                   folder_name, label_name='label_1st', batchnorm=0, lr_mult_distro=[1, 1, 1], new=0):
    # write the net prototxt files out
    if new == 6:
        print 'weight'
        with open('%s/pose_train_test.prototxt' % sub_dir, 'w') as f:
            print 'writing train_test prototxt'
            str_to_write = setLayers_twoBranches(source, batch_size, layername, kernel, stride, outCH, label_name,
                                                 transform_param_in, deploy=False, batchnorm=batchnorm,
                                                 lr_mult_distro=lr_mult_distro)
            f.write(str_to_write)

        with open('%s/pose_deploy.prototxt' % sub_dir, 'w') as f:
            print 'writing deploy prototxt'
            str_to_write = str(
                setLayers_twoBranches('', 0, layername, kernel, stride, outCH, label_name, transform_param_in,
                                      deploy=True, batchnorm=batchnorm, lr_mult_distro=lr_mult_distro))
            f.write(str_to_write)
...
def calcAndWriteStat(sub_dir, layername, kernel, stride, outCH, args):
    ...
    for l in range(len(layername)):
        ...
        print 'LAYER %s | mem: %d, flop: %d, nparam: %d, current_ch: %d' % (
        layername[l], mem, flop - last_flop, nparam, current_ch)
        last_flop = flop
...
if __name__ == "__main__":
    ...
    if (exp == 1):
        ...
        base_folder = '/home/long/github/Res50CPN-Realtime_Multi-Person_Pose_Estimation/training/' + directory + 'model'
        dataFolder = '/media/D/Realtime_Multi-Person_Pose_Estimation/training/COCO_kpt/lmdb_trainVal'
        ...
        for nc in range(0, 1):
            layername = ['V', 'V', 'P'] * 2 + ['V'] * 4 + ['P'] + ['V'] * 2 + ['C'] * 2 + ['$'] + ['C2'] * 3 + [
                                                                                                                   'C2'] * 2 + [
                            'L2']  # first-stage
            kernel = [3, 3, 2] * 2 + [3] * 4 + [2] + [3] * 2 + [3] * 2 + [0] + [3] * 3 + [1] * 2 + [0]  # first-stage
            outCH = [64] * 3 + [128] * 3 + [256] * 4 + [256] + [512] * 2 + [256] + [128] + [0] + [128] * 3 + [512] + [
                np * 2] + [0]  # first-stage
            ...
            writePrototxts(dataFolder, sub_dir, batch_size, layername, kernel, stride, outCH, transform_param, base_lr,
                           d_caffemodel, label_name, 0, lr_mult_distro, 6)
